                    ...
                    <div class="invoice-section">
                        ...
                        <div class="section-body">
                            <div class="form-row">
                                ...
                                <div class="form-field">
                                    <label>IF</label>
                                    <input type="text" id="editClientIFSimpleMRY" placeholder="Identifiant Fiscal (chiffres uniquement)" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                                </div>
                            </div>
                        </div>
                    </div>
...
async function loadInvoiceDataSimpleMRY(invoiceId) {
    try {
        ...
        document.getElementById('editClientIFSimpleMRY').value = invoice.client_if || '';
        ...
        document.getElementById('editDocumentDateSimpleMRY').value = window.safeDateString ? window.safeDateString(invoice.document_date) : (invoice.document_date ? invoice.document_date.split('T')[0] : '');
...
function displayClientsListEditSimpleMRY() {
    ...
    dropdown.innerHTML = filteredClientsEditSimpleMRY.slice(0, 10).map(client => `
        <div class="dropdown-item" style="display: flex; justify-content: space-between; align-items: center;">
            <div style="flex: 1;" onmousedown="selectClientEditSimpleMRY('${client.nom.replace(/'/g, "\\'")}', '${client.ice}', '${client.client_if || ''}')">
...
window.selectClientEditSimpleMRY = function (nom, ice, clientIf) {
    document.getElementById('editClientNomSimpleMRY').value = nom;
    document.getElementById('editClientICESimpleMRY').value = ice;
    document.getElementById('editClientIFSimpleMRY').value = clientIf || '';
    const dropdown = document.getElementById('clientsDropdownEditSimpleMRY');
    if (dropdown) dropdown.style.display = 'none';
}
    ...
    try {
        ...
        const formData = {
            client: {
                ...
                IF: document.getElementById('editClientIFSimpleMRY')?.value || ''
            },
            document: {
