        ...
        <div class="desktop-app">
            ...
            <div class="window-content" style="display: flex; align-items: center; justify-content: center; min-height: calc(100vh - 60px); background: linear-gradient(135deg, #1e1e1e 0%, #2d2d30 100%);">
                <div style="max-width: 1200px; width: 100%; padding: 2rem;">
                    ...
                    <!-- Loading Spinner -->
                    <div id="yearLoadingSpinnerMulti" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem;">
                    ...
                    <div id="yearsCardsGridMulti" style="display: none; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 2rem; margin-bottom: 2rem;">
                        <!-- Year cards will be inserted here -->
                    </div>

                    <!-- Back Button -->
                    <div style="text-align: center; margin-top: 3rem;">
                        <button onclick="router.navigate('/dashboard-multi')" style="padding: 1rem 2rem; background: #3e3e42; color: #ffffff; border: 2px solid #3e3e42; border-radius: 8px; cursor: pointer; font-size: 1rem; font-weight: 600; transition: all 0.3s;" onmouseover="this.style.background='#4e4e52'; this.style.borderColor='#667eea';" onmouseout="this.style.background='#3e3e42'; this.style.borderColor='#3e3e42';">
...
// Load and display available years for MULTI
window.loadYearSelectorMulti = async function() {
    const loadingSpinner = document.getElementById('yearLoadingSpinnerMulti');
    const cardsGrid = document.getElementById('yearsCardsGridMulti');
    
    try {
        // Check if user has saved preference to skip selection
        const rememberYear = localStorage.getItem('multi_remember_year');
        const savedYear = localStorage.getItem('multi_selected_year');
        
        if (rememberYear === 'true' && savedYear) {
            // Skip selection modal and go directly to invoices list
        ...
        // Render year cards
        renderYearCardsSelectorMulti(availableYears, yearCounts, allInvoices.length);
...
// Render year cards in selector for MULTI
function renderYearCardsSelectorMulti(years, yearCounts, totalCount) {
    const container = document.getElementById('yearsCardsGridMulti');
    if (!container) return;
    ...
    // Add "All Years" card
    cardsHTML += `
        <div onclick="selectYearAndNavigateMulti('', ${totalCount})" 
    ...
    years.forEach(year => {
        ...
        cardsHTML += `
            <div onclick="selectYearAndNavigateMulti('${year}', ${count})" 
...
// Select year and navigate to invoices list for MULTI
window.selectYearAndNavigateMulti = function(year, count) {
        ...
        <div style="background: #2d2d30; border: 2px solid #667eea; border-radius: 16px; padding: 2.5rem; max-width: 500px; width: 90%; box-shadow: 0 8px 32px rgba(0,0,0,0.5);">
            ...
            <div style="background: #1e1e1e; border: 1px solid #3e3e42; border-radius: 8px; padding: 1.5rem; margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; cursor: pointer; color: #cccccc; font-size: 1rem;">
                    <input type="checkbox" id="rememberYearCheckboxMulti" style="width: 20px; height: 20px; margin-right: 1rem; cursor: pointer;">
...
// Confirm year selection and navigate for MULTI
window.confirmYearSelectionMulti = function(year) {
    const rememberCheckbox = document.getElementById('rememberYearCheckboxMulti');
    
    // Save to localStorage if checkbox is checked
    if (rememberCheckbox && rememberCheckbox.checked) {
        localStorage.setItem('multi_selected_year', year);
        localStorage.setItem('multi_remember_year', 'true');
    } else {
    ...
    sessionStorage.setItem('multi_current_year', year);
