...
// Main function to download SAAISS Devis PDF
window.downloadSAAISSDevisPDF = async function(invoiceId) {
    try {
        console.log('📥 Generating SAAISS PDF for invoice:', invoiceId);
    ...
    return new Promise((resolve) => {
        ...
        if (lastDevisNumber) {
            ...
            if (match) {
                ...
                const prefix = lastDevisNumber.substring(0, lastDevisNumber.length - match[1].length);
                nextDevisNumber = prefix + String(lastNum + 1).padStart(match[1].length, '0') + '/' + currentYear;
            }
        } else {
    ...
    try {
        ...
        const addSAAISSFooter = (pageNum, totalPages) => {
            ...
            if (signatureImg) {
                ...
                const signatureX = (pageWidth / 2) - (signatureWidth / 2) - 20; // Center horizontally, slightly left
