                ...
                <form id="invoiceForm" class="invoice-form">
                    ...
                    <div class="invoice-section">
                        ...
                        <div class="section-body">
                            ...
                            <div id="adminFieldsMRY" style="display: none;">
                                <!-- Featured Checkbox -->
                                <div class="form-field" style="margin-bottom: 1.5rem;">
                                    <label style="display: flex; align-items: center; cursor: pointer; user-select: none;">
                                        <input type="checkbox" id="invoiceFeaturedMRY" 
                                ...
                                <div class="form-field" style="margin-bottom: 1.5rem;">
                                    ...
                                    <textarea id="invoicePrivateNotesMRY" rows="3" 
...
function displayClientsList() {
    ...
    dropdown.innerHTML = filteredClients.slice(0, 10).map(client => `
        <div class="dropdown-item" style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <button class="delete-client-btn" onclick="event.stopPropagation(); deleteClient(${client.id}, '${client.nom.replace(/'/g, "\\'")}');" 
...
// Show custom delete error modal for MRY
function showDeleteErrorModalMRY(clientName, errorMessage) {
    ...
    modal.querySelector('#confirmDeleteBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                searchClients(document.getElementById('clientNom').value);
            } else {
                showDeleteErrorModalMRY(clientName, result.error || 'فشل حذف الزبون');
            }
        } catch (error) {
            console.error('Error deleting client:', error);
            showDeleteErrorModalMRY(clientName, 'حدث خطأ أثناء حذف الزبون');
        }
    });
}
...
async function handleInvoiceSubmit(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            const isFeatured = document.getElementById('invoiceFeaturedMRY')?.checked ? 1 : 0;
            const privateNotes = document.getElementById('invoicePrivateNotesMRY')?.value?.trim() || '';
            if (isFeatured || privateNotes) {
                await window.electron.db.updateInvoiceMetadata(invoiceId, {
...
window.initInvoiceFormPage = function () {
    ...
    const adminFields = document.getElementById('adminFieldsMRY');
