...
window.generateSituationMensuelle = async function (clientId, month, year, sortBy = 'date_asc', selectedInvoiceIds = [], manualInvoices = [], includeOrder = true, includeBL = true, includeBC = true) {
    try {
        ...
        allInvoices.forEach((inv, index) => {
            ...
            doc.setFontSize(8);
            doc.setFont(undefined, 'normal');
            doc.text(formatAmount(hVal) + ' DH', 188, currentY + 2.5, { align: 'right' });
        ...
        doc.setFontSize(8);
        doc.text(`${formatAmount(totalTVA)} DH`, 192, currentY + 5.5, { align: 'right' });
        ...
        // Save PDF
        const filename = `Situation_${client.nom.replace(/\s+/g, '_')}_${monthNames[month]}_${year}_CHAIMAE.pdf`;
        doc.save(filename);
