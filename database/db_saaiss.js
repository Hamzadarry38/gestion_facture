...
const dbPath = path.join(dbDir, 'saaiss.db');
...
async function initDatabase() {
    try {
        ...
        if (fs.existsSync(dbPath)) {
            ...
            console.log('✅ [SAAISS] Database loaded successfully');
        } else {
            db = new SQL.Database();
            console.log('✅ [SAAISS] Database created successfully');
        }
        
        // Create SAAISS devis numbers table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_saaiss_devis_year ON saaiss_devis_numbers(year, devis_number)`);
        
        // Create SAAISS PDF files table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_saaiss_pdf_year ON saaiss_pdf_files(year, devis_number)`);
        
        console.log('✅ [SAAISS] Database tables created successfully');
        ...
        console.error('❌ [SAAISS] Database initialization failed:', error);
        throw error;
    }
}
...
// SAAISS Devis Numbers Operations
const saaissDevisOps = {
    // Check if devis number exists
    exists: (devisNumber, year = null) => {
        let query = 'SELECT COUNT(*) as count FROM saaiss_devis_numbers WHERE devis_number = ?';
    ...
    add: (devisNumber, year = new Date().getFullYear()) => {
        ...
        if (saaissDevisOps.exists(devisNumber, year)) {
            throw new Error(`رقم الـ Devis "${devisNumber}" موجود مسبقاً في سنة ${year}`);
        }
        
        db.run(`
            INSERT INTO saaiss_devis_numbers (devis_number, year, created_at, used_at)
    ...
    delete: (devisNumber, year = null) => {
        let query = 'DELETE FROM saaiss_devis_numbers WHERE devis_number = ?';
        let params = [devisNumber];
    ...
    getMax: (year = new Date().getFullYear()) => {
        const allDevis = saaissDevisOps.getByYear(year);
    ...
    // Clear all data
    clearAll: () => {
        db.run('DELETE FROM saaiss_devis_numbers');
        db.run('DELETE FROM sqlite_sequence WHERE name = "saaiss_devis_numbers"');
        saveDatabase();
        return true;
    }
};

// SAAISS PDF Files Operations
const saaissPdfOps = {
    // Save PDF file path
    savePdfPath: (devisNumber, year, filePath, createdBy = 'Unknown') => {
        try {
            db.run(`
                INSERT OR REPLACE INTO saaiss_pdf_files (devis_number, year, file_path, created_by, created_at)
                VALUES (?, ?, ?, ?, datetime('now'))
