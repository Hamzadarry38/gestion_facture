...
const dbPath = path.join(dbDir, 'smarts.db');
...
async function initDatabase() {
    try {
        ...
        if (fs.existsSync(dbPath)) {
            ...
            console.log('✅ [SMART SERVICES] Database loaded successfully');
        } else {
            db = new SQL.Database();
            console.log('✅ [SMART SERVICES] Database created successfully');
        }

        // Create SMART SERVICES devis numbers table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_smarts_devis_year ON smarts_devis_numbers(year, devis_number)`);

        // Create SMART SERVICES PDF files table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_smarts_pdf_year ON smarts_pdf_files(year, devis_number)`);

        console.log('✅ [SMART SERVICES] Database tables created successfully');
        ...
        console.error('❌ [SMART SERVICES] Database initialization failed:', error);
        throw error;
    }
}
...
// SMART SERVICES Devis Numbers Operations
const smartSDevisOps = {
    // Check if devis number exists
    exists: (devisNumber, year = null) => {
        let query = 'SELECT COUNT(*) as count FROM smarts_devis_numbers WHERE devis_number = ?';
    ...
    add: (devisNumber, year = new Date().getFullYear()) => {
        devisNumber = devisNumber.trim(); // Ensure no leading/trailing spaces
        // Check if already exists
        if (smartSDevisOps.exists(devisNumber, year)) {
            throw new Error(`رقم الـ Devis "${devisNumber}" موجود مسبقاً في سنة ${year}`);
        }

        db.run(`
            INSERT INTO smarts_devis_numbers (devis_number, year, created_at, used_at)
    ...
    delete: (devisNumber, year = null) => {
        let query = 'DELETE FROM smarts_devis_numbers WHERE devis_number = ?';
        let params = [devisNumber];
    ...
    getMax: (year = new Date().getFullYear()) => {
        const allDevis = smartSDevisOps.getByYear(year);
        ...
        allDevis.forEach(item => {
            // Relaxed regex: allows spaces, handles number/year
    ...
    // Clear all data
    clearAll: () => {
        db.run('DELETE FROM smarts_devis_numbers');
        db.run('DELETE FROM sqlite_sequence WHERE name = "smarts_devis_numbers"');
        saveDatabase();
        return true;
    }
};

// SMART SERVICES PDF Files Operations
const smartSPdfOps = {
    // Save PDF file path
    savePdfPath: (devisNumber, year, filePath, createdBy = 'Unknown') => {
        try {
            db.run(`
                INSERT OR REPLACE INTO smarts_pdf_files (devis_number, year, file_path, created_by, created_at)
                VALUES (?, ?, ?, ?, datetime('now'))
