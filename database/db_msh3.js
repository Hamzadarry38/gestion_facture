...
const dbPath = path.join(dbDir, 'msh3.db');
...
async function initDatabase() {
    try {
        ...
        if (fs.existsSync(dbPath)) {
            ...
            console.log('✅ [MSH3 SERVICES] Database loaded successfully');
        } else {
            db = new SQL.Database();
            console.log('✅ [MSH3 SERVICES] Database created successfully');
        }

        // Create MSH3 devis numbers table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_msh3_devis_year ON msh3_devis_numbers(year, devis_number)`);

        // Create MSH3 PDF files table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_msh3_pdf_year ON msh3_pdf_files(year, devis_number)`);

        console.log('✅ [MSH3 SERVICES] Database tables created successfully');
        ...
        console.error('❌ [MSH3 SERVICES] Database initialization failed:', error);
        throw error;
    }
}
...
// MSH3 Devis Numbers Operations
const msh3DevisOps = {
    // Check if devis number exists
    exists: (devisNumber, year = null) => {
        let query = 'SELECT COUNT(*) as count FROM msh3_devis_numbers WHERE devis_number = ?';
    ...
    add: (devisNumber, year = new Date().getFullYear()) => {
        ...
        if (msh3DevisOps.exists(devisNumber, year)) {
            throw new Error(`رقم الـ Devis "${devisNumber}" موجود مسبقاً في سنة ${year}`);
        }

        db.run(`
            INSERT INTO msh3_devis_numbers (devis_number, year, created_at, used_at)
    ...
    delete: (devisNumber, year = null) => {
        let query = 'DELETE FROM msh3_devis_numbers WHERE devis_number = ?';
        let params = [devisNumber];
    ...
    getMax: (year = new Date().getFullYear()) => {
        const allDevis = msh3DevisOps.getByYear(year);
    ...
    // Clear all data
    clearAll: () => {
        db.run('DELETE FROM msh3_devis_numbers');
        db.run('DELETE FROM sqlite_sequence WHERE name = "msh3_devis_numbers"');
        saveDatabase();
        return true;
    }
};

// MSH3 PDF Files Operations
const msh3PdfOps = {
    // Save PDF file path
    savePdfPath: (devisNumber, year, filePath, createdBy = 'Unknown') => {
        try {
            db.run(`
                INSERT OR REPLACE INTO msh3_pdf_files (devis_number, year, file_path, created_by, created_at)
                VALUES (?, ?, ?, ?, datetime('now'))
