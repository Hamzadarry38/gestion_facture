...
const dbPath = path.join(dbDir, 'benali.db');
...
async function initDatabase() {
    try {
        ...
        if (fs.existsSync(dbPath)) {
            ...
            console.log('✅ [BEN ALI] Database loaded successfully');
        } else {
            db = new SQL.Database();
            console.log('✅ [BEN ALI] Database created successfully');
        }

        // Create BEN ALI devis numbers table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_benali_devis_year ON benali_devis_numbers(year, devis_number)`);

        // Create BEN ALI PDF files table
        db.run(`
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_benali_pdf_year ON benali_pdf_files(year, devis_number)`);

        console.log('✅ [BEN ALI] Database tables created successfully');
        ...
        console.error('❌ [BEN ALI] Database initialization failed:', error);
        throw error;
    }
}
...
// BEN ALI Devis Numbers Operations
const benaliDevisOps = {
    // Check if devis number exists
    exists: (devisNumber, year = null) => {
        let query = 'SELECT COUNT(*) as count FROM benali_devis_numbers WHERE devis_number = ?';
    ...
    add: (devisNumber, year = new Date().getFullYear()) => {
        ...
        if (benaliDevisOps.exists(devisNumber, year)) {
            throw new Error(`رقم الـ Devis "${devisNumber}" موجود مسبقاً في سنة ${year}`);
        }

        db.run(`
            INSERT INTO benali_devis_numbers (devis_number, year, created_at, used_at)
    ...
    delete: (devisNumber, year = null) => {
        let query = 'DELETE FROM benali_devis_numbers WHERE devis_number = ?';
        let params = [devisNumber];
    ...
    getMax: (year = new Date().getFullYear()) => {
        const allDevis = benaliDevisOps.getByYear(year);
    ...
    // Clear all data
    clearAll: () => {
        db.run('DELETE FROM benali_devis_numbers');
        db.run('DELETE FROM sqlite_sequence WHERE name = "benali_devis_numbers"');
        saveDatabase();
        return true;
    }
};

// BEN ALI PDF Files Operations
const benaliPdfOps = {
    // Save PDF file path
    savePdfPath: (devisNumber, year, filePath, createdBy = 'Unknown') => {
        try {
            db.run(`
                INSERT OR REPLACE INTO benali_pdf_files (devis_number, year, file_path, created_by, created_at)
                VALUES (?, ?, ?, ?, datetime('now'))
