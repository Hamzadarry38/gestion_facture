        ...
        try {
            ...
            if (attTableInfo.length > 0) {
                ...
                if (fileDataCol && fileDataCol[3] === 1) {
                    console.log('🔄 [CHAIMAE DB] Migrating attachments table to allow NULL file_data...');
                    ...
                    saveDatabase();
                    console.log('✅ [CHAIMAE DB] Attachments table migrated successfully');
                }
            }
        } catch (error) {
            console.log('ℹ️ [CHAIMAE DB] Note: Could not migrate attachments table:', error.message);
        }
...
const invoiceOps = {
    ...
    getAll: () => {
        ...
        const columns = result[0].columns;
        console.log('🔍 [DB CHAIMAE] getAll Columns detected:', columns);
        console.log('🔍 [DB CHAIMAE] Mapped total_ttc index:', columns.indexOf('total_ttc'));

        const invoices = result[0].values.map((row, rIdx) => {
            // Helper to safely get value by column name
            const getVal = (col) => row[columns.indexOf(col)];

            // Debug first row
            if (rIdx === 0) {
                console.log('🔍 [DB CHAIMAE] First Row total_ttc RAW value:', getVal('total_ttc'));
            }
