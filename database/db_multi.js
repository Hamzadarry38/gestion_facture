        ...
        try {
            ...
            if (attTableInfo.length > 0) {
                ...
                if (fileDataCol && fileDataCol[3] === 1) {
                    console.log('🔄 [MULTI DB] Migrating attachments table to allow NULL file_data...');
                    ...
                    saveDatabase();
                    console.log('✅ [MULTI DB] Attachments table migrated successfully');
                }
            }
        } catch (error) {
            console.log('ℹ️ [MULTI DB] Note: Could not migrate attachments table:', error.message);
        }
...
const invoiceOps = {
    ...
    getById: (id) => {
        ...
        const invoice = {
            ...
            creation_method: row[row.length - 2] || 'normal', // Handle potential index shifts if columns added
            client_nom: row[26],
            client_ice: row[27]
        };
    ...
    getAll: (companyCode = null) => {
        ...
        return result[0].values.map(row => {
            ...
            return {
                ...
                creation_method: row[row.length - 2] || 'normal',
                products: products
            };
        });
    },
