...
db_path = os.path.join(appdata_path, 'invoices_chaimae.db')
...
def create_invoice(conn, sequential_id):
    """Create an invoice for CHAIMAE company - ALPHA CONSTRUCTION - November 2025"""
    cursor = conn.cursor()
    ...
    doc_type = random.choice(['facture', 'facture', 'facture', 'bon_livraison', 'bon_commande'])
    ...
    if doc_type == 'facture':
        ...
        doc_bon_livraison = f"BL{random.randint(1000, 9999)}" if random.random() > 0.5 else None
    elif doc_type == 'bon_livraison':
        doc_numero = f"BL{sequential_id}/{year}"
        doc_numero_order = None
        doc_bon_livraison = None
    else:  # bon_commande
        doc_numero = f"BC{sequential_id}/{year}"
        doc_numero_order = None
        doc_bon_livraison = None
    ...
    cursor.execute('''
        INSERT INTO invoices (
            ...
            document_numero, document_numero_Order, document_bon_de_livraison,
            year, sequential_id, total_ht, tva_rate, montant_tva, total_ttc
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 20, 0, 0)
    ''', (
        client_id, doc_type, invoice_date.strftime('%Y-%m-%d'),
        doc_numero, doc_numero_order, doc_bon_livraison, year, sequential_id
    ))
...
def main():
    print("🚀 Génération de 100 factures pour ALPHA CONSTRUCTION - Novembre 2025 (CHAIMAE)")
    print(f"📁 Chemin: {db_path}")
    
    if not os.path.exists(db_path):
        print(f"❌ Base de données non trouvée!")
        print(f"   Veuillez d'abord lancer l'application et accéder à CHAIMAE.")
        return
