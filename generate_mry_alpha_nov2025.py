...
db_path = os.path.join(appdata_path, 'invoices.db')
...
def get_next_sequential_id(conn, year):
    ...
    cursor.execute('''
        ...
        WHERE year = ? AND company_code = 'MRY'
    ''', (year,))
    result = cursor.fetchone()
    return (result[0] + 1) if result[0] else 1

def create_invoice(conn, sequential_id):
    """Create an invoice for MRY company - ALPHA CONSTRUCTION - November 2025"""
    cursor = conn.cursor()
    ...
    cursor.execute('''
        INSERT INTO invoices (
            ...
            document_numero, document_numero_devis,
            year, sequential_id, total_ht, tva_rate, montant_tva, total_ttc
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 20, 0, 0)
    ''', (
        'MRY', client_id, doc_type, invoice_date.strftime('%Y-%m-%d'),
        doc_numero, doc_numero_devis, year, sequential_id
    ))
...
def main():
    print("🚀 Génération de 100 factures pour ALPHA CONSTRUCTION - Novembre 2025 (MRY)")
